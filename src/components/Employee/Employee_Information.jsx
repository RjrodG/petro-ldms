    ...
    <div className="employee-modal-backdrop" onClick={closeModal}>
      <div className="employee-modal" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSave}>
              ...
              <tbody>
                  ...
                  <td>
                    {modal.type === 'view' ? (
                      <span style={{ fontWeight: 'bold' }}>
                        {positionOptions.find(pos => pos.pos_id === modal.employee.pos_id)?.pos_name || 'N/A'}
                      </span>
                    ) : (
                      <select
                        name="pos_id"
                        value={modal.employee.pos_id || ''}
                        onChange={handleChange}
                        required
                      >
                        <option value="">Select Position</option>
                        {positionOptions.map(pos => (
                          <option key={pos.pos_id} value={pos.pos_id}>{pos.pos_name}</option>
                        ))}
                      </select>
                    )}
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    {modal.type === 'view' ? (
                      <span style={{ fontWeight: 'bold' }}>
                        {departmentOptions.find(dep => dep.dep_id === modal.employee.dep_id)?.dep_name || 'N/A'}
                      </span>
                    ) : (
                      <select
                        name="dep_id"
                        value={modal.employee.dep_id}
                        onChange={handleChange}
                        required
                      >
                ...
                <tr>
                  ...
                  <td>
                    {modal.type === 'view' ? (
                      <span style={{ fontWeight: 'bold' }}>
                        {statusOptions.find(stat => stat.stat_id === modal.employee.stat_id)?.stat_name || 'N/A'}
                      </span>
                    ) : (
                      <select
                        name="stat_id"
                        value={modal.employee.stat_id}
                        onChange={handleChange}
                        required
                      >
                        <option value="">Select Status</option>
                        {statusOptions.map(stat => (
                          <option key={stat.stat_id} value={stat.stat_id}>{stat.stat_name}</option>
                        ))}
                      </select>
                    )}
                  </td>
                ...
                <tr>
                  ...
                  <td>
                    {modal.type === 'view' ? (
                      <span style={{ fontWeight: 'bold' }}>
                        {classificationOptions.find(clas => clas.class_id === modal.employee.class_id)?.class_name || 'N/A'}
                      </span>
                    ) : (
                      <select
                        name="class_id"
                        value={modal.employee.class_id}
                        onChange={handleChange}
                        required
                      >
                        <option value="">Select Classification</option>
                        {classificationOptions.map(clas => (
                          <option key={clas.class_id} value={clas.class_id}>{clas.class_name}</option>
                        ))}
                      </select>
                    )}
                  </td>
                  ...
                  <>
                    <tr>
                      ...
                      <td>
                        {modal.type === 'view' ? (
                          <span style={{ fontWeight: 'bold' }}>
                            {professionOptions.find(prof => prof.prof_id === modal.employee.prof_id)?.prof_name || 'N/A'}
                          </span>
                        ) : (
                          <select
                            name="prof_id"
                            value={modal.employee.prof_id}
                            onChange={handleChange}
                            required
                          >
                            <option value="">Select Profession</option>
                            {professionOptions.map(prof => (
                              <option key={prof.prof_id} value={prof.prof_id}>{prof.prof_name}</option>
                            ))}
                          </select>
                        )}
                      </td>
          ...
          {/* PART II & PART III TAB CONTROL, IN-HOUSE, OUT-HOUSE, SALN AND IPCR */}
          <div className="idp-tab-control">
            <div className="idp-tabs">
              <button
                type="button"
                className={selectedTab === 'idp' ? 'idp-tab active' : 'idp-tab'}
                onClick={() => setSelectedTab('idp')}>
                Individual Development Plan
              </button>
              <button
                type="button"
                className={selectedTab === 'inhouse' ? 'idp-tab active' : 'idp-tab'}
                onClick={() => setSelectedTab('inhouse')}>
                In-House Training (LD/Non-LD)
              </button>
              <button
                type="button"
                className={selectedTab === 'outhouse' ? 'idp-tab active' : 'idp-tab'}
                onClick={() => setSelectedTab('outhouse')}>
                Out-House Training (OT/OB)
              </button>
              <button
                type="button"
                className={selectedTab === '' ? 'idp-tab active' : 'idp-tab'}
          ...
          <div className="modal-actions">
            {modal.type !== 'view' && (
              <button className="save-employee-btn" type="submit">
                {modal.type === 'add'
              ...
              <button className="close-employee-btn" type="button" onClick={closeModal}>
